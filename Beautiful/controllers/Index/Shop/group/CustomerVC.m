...
@interface CustomerVC ()<UIScrollViewDelegate>
{
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setNavigationBar:@"顾客团购"];
    [self addBackButton];
    
    
    scrollvi = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, Width, Height)];
...
-(void)wheitview{
    ...
    UIButton * tailbtn = [UIButton buttonWithType:UIButtonTypeCustom];
    tailbtn.frame = CGRectMake(0, Height-50, Width, 50);
    tailbtn.backgroundColor = UIColorFromRGB(0x31cdaa);
    [tailbtn addTarget:self action:@selector(tailbtnCilk:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:tailbtn];
    
    UILabel *muchlab = [[UILabel alloc]initWithFrame:CGRectMake(Width/2-25,8,50, 20)];
    muchlab.text = @"￥49";
    muchlab.textColor = UIColorFromRGB(0xffffff);
    muchlab.font = [UIFont systemFontOfSize:19];
    muchlab.textAlignment = NSTextAlignmentCenter;
    [tailbtn addSubview:muchlab];
    
    UILabel *partakelab = [[UILabel alloc]initWithFrame:CGRectMake(Width/2-40,30,80, 15)];
    partakelab.text = @" 参与10人团";
    partakelab.textColor = UIColorFromRGB(0xffffff);
    partakelab.font = [UIFont systemFontOfSize:13];
    partakelab.textAlignment = NSTextAlignmentCenter;
    [tailbtn addSubview:partakelab];
...
-(void)lable{
    ...
    UILabel *univalencelab = [[UILabel alloc]initWithFrame:CGRectMake(105,57, 75, 20)];
...
-(void)tailbtnCilk:(UIButton *)tailbtn{
    NSLog(@"确认发布");
 }
